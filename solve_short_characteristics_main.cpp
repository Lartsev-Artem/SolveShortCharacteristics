...
//name_file_out_faces:
	std::unique_ptr<FILE, int(*)(FILE*)> file_out_id(fopen((name_file_out_faces + ".bin").c_str(), "rb"), fclose);
	if (!file_out_id) PRINTF("Error file_out_id\n")
		for (int h = 0; h < N; ++h)
		{
			int n;
			fread_unlocked(&n, sizeof(int), 1, file_out_id.get());
			grid[h].out_id.resize(n);
			fread_unlocked(grid[h].out_id.data(), sizeof(int), n, file_out_id.get());
		}
	fclose(file_out_id.get());

//name_file_in_faces
	std::unique_ptr<FILE, int(*)(FILE*)> file_in_id(fopen((name_file_in_faces + ".bin").c_str(), "rb"), fclose);
	if (!file_in_id) PRINTF("Error wfile_out_id\n")

		for (int h = 0; h < N; ++h)
		{
			int n;
			fread_unlocked(&n, sizeof(int), 1, file_in_id.get());
			grid[h].in_id.resize(n);
			fread_unlocked(grid[h].in_id.data(), sizeof(int), n, file_in_id.get());
		}
...
//name_file_x
	...
	if (!file_x) PRINTF("Error wfile_x\n")

		for (int h = 0; h < N; ++h)
		{
			int n;
			fread_unlocked(&n, sizeof(int), 1, file_x.get());
			grid[h].x.resize(n);
			for (int i = 0; i < n; i++)
				fread_unlocked(grid[h].x[i].data(), sizeof(double), 3, file_x.get());
		}
	fclose(file_x.get());


//name_file_local_x0
	std::unique_ptr<FILE, int(*)(FILE*)> file_x0_local(fopen((name_file_local_x0 + ".bin").c_str(), "rb"), fclose);
	if (!file_x0_local) PRINTF("Error wfile_x\n")

		for (int h = 0; h < N; ++h)
		{
			int n;
			fread_unlocked(&n, sizeof(int), 1, file_x0_local.get());
			grid[h].x0_loc.resize(n);
			for (int i = 0; i < n; i++)
				fread_unlocked(grid[h].x0_loc[i].data(), sizeof(double), 2, file_x0_local.get());
		}
	fclose(file_x0_local.get());

//name_file_s
	std::unique_ptr<FILE, int(*)(FILE*)> file_s(fopen((name_file_s + ".bin").c_str(), "rb"), fclose);
	if (!file_s) PRINTF("Error wfile_s\n")

		for (int h = 0; h < N; ++h)
		{
			int n;
			fread_unlocked(&n, sizeof(int), 1, file_s.get());
			grid[h].s.resize(n);
			fread_unlocked(grid[h].s.data(), sizeof(double), n, file_s.get());
...
int mainOldGood(int argc, char* argv[])
{
	...
	*/
	...
	vector<IntId> full_sorted_id_cell;
	{
		...
		int n;
		fread_unlocked(&n, sizeof(int), 1, file_graph.get());
		full_sorted_id_cell.resize(n);
		fread_unlocked(full_sorted_id_cell.data(), sizeof(IntId), n, file_graph.get());
		fclose(file_graph.get());
	}

	std::unique_ptr<FILE, int(*)(FILE*)> file_count_out(fopen((name_file_count_out_faces + "One.bin").c_str(), "wb"), fclose);
	if (!file_count_out) { printf("file_graph is not opened for writing\n"); return 1; }

	std::unique_ptr<FILE, int(*)(FILE*)> file_out(fopen((name_file_out_faces + "One.bin").c_str(), "wb"), fclose);
	if (!file_out) { printf("file_graph is not opened for writing\n"); return 1; }

	std::unique_ptr<FILE, int(*)(FILE*)> file_in(fopen((name_file_in_faces + "One.bin").c_str(), "wb"), fclose);
	if (!file_in) { printf("file_graph is not opened for writing\n"); return 1; }

	std::unique_ptr<FILE, int(*)(FILE*)> file_x(fopen((name_file_x + "One.bin").c_str(), "wb"), fclose);
	if (!file_x) { printf("file_graph is not opened for writing\n"); return 1; }

	std::unique_ptr<FILE, int(*)(FILE*)> file_x0(fopen((name_file_local_x0 + "One.bin").c_str(), "wb"), fclose);
	if (!file_x0) { printf("file_graph is not opened for writing\n"); return 1; }

	std::unique_ptr<FILE, int(*)(FILE*)> file_s(fopen((name_file_s + "One.bin").c_str(), "wb"), fclose);
		...
		Type _clock = -omp_get_wtime();
		{
			...
			for (int num_direction = 0; num_direction < count_directions; ++num_direction)
			{
				...
				for (int h = 0; h < count_cells; ++h) {
					...
					fwrite_unlocked(&n_out, sizeof(int), 1, file_count_out.get());
					countOutC++;
					...
					for (int i = 0; i < n_out; ++i) {
						...
						fwrite_unlocked(&num_out_face, sizeof(int), 1, file_out.get());
						countOut++;
...
//CalculateNodeValue:
							...
							fwrite_unlocked(&num_in_face, sizeof(int), 1, file_in.get());
							countIn++;
							
							Type I_x0 = CalculateIllumeOnInnerFace(num_cell, num_in_face, start_3 + i + num_node, x, grid);
							if (flag) { fwrite_unlocked(x0_local_test.data(), sizeof(Type), 2, file_x0.get()); 
							countX0++;
							}
